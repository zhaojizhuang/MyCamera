...
import com.zjz.util.CameraUtils;
...
public class CameraInterface {
	...
	private void initCamera(float previewRate) {
		...
		if (mCamera != null) {
			...
			Size pictureSize = CamParaUtils.getPropPreview_Picture_Size(mParams.getSupportedPictureSizes(), previewRate,
					800);
			mParams.setPictureSize(pictureSize.width, pictureSize.height);

			Size previewSize = CamParaUtils.getPropPreview_Picture_Size(mParams.getSupportedPreviewSizes(), previewRate,
					800);
			...
			Log.d(TAG, "最终设置:PreviewSize--With = " + mParams.getPreviewSize().width + "Height = "
					+ mParams.getPreviewSize().height);
			Log.d(TAG, "最终设置:PictureSize--With = " + mParams.getPictureSize().width + "Height = "
	...
	PictureCallback mJpegPictureCallback = new PictureCallback() {
		...
		public void onPictureTaken(byte[] data, Camera camera) {
			...
			if (null != b) {
				...
				FileUtils.saveBitmap(rotaBitmap);
				FileUtils.scanSdCard(MyApplication.getContext());
			}
			// 再次进入预览
			mCamera.startPreview();
			isPreviewing = true;
